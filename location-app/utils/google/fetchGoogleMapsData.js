...
export function fecthAutoCompleteGoogleMaps(query) {
	const autocompleteApi = `${AUTOCOMPLETE_PLACE}${query}&key=${API_KEY}`;
...
export async function fetchPlaceDetails(placeId) {
	return await fetchData(`${DETAILS_PLACE}${placeId}&key=${API_KEY}`);
}

export async function fetchPlaceOrGeoGoogleMaps(coords) {
	const placesApi = `${PLACE_SEARCH}${coords}&${RADIUS}&key=${API_KEY}`;
	const geoCodeApi = `${GEOCODE}${coords}&${RADIUS}&key=${API_KEY}`;
